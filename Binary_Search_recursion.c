...
int binary_search(data_node *array,int start,int end,int value)
{
int mid=start+(end-start)/2;
if(array[mid].data==value) return mid;
else
   if(array[mid].data<value) binary_search(array,mid+1,end,value);
   else binary_search(array,start,mid-1,value);
...
int  main()
{
    ...
    location=binary_search(initial_array,0,array_length-1,value);
    /* 打印 */
